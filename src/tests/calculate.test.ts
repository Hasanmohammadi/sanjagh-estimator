...
describe("calculateEstimate", () => {
  ...
  describe("محاسبه لیتر رنگ", () => {
    ...
    it("با تعداد دست بیشتر، لیتر بیشتری حساب کنه", () => {
      ...
      const result3 = calculateEstimate(
        [threeCoats],
        defaultPaintPrices,
        defaultPricePerLiter,
        true,
      );
      const result2 = calculateEstimate(
        [twoCoats],
        defaultPaintPrices,
        defaultPricePerLiter,
        true,
  ...
  describe("محاسبه هزینه طبق فرمول سند", () => {
    ...
    it("با مصالح از بدون مصالح گرون‌تر باشه", () => {
      const withMat = calculateEstimate(
        [simpleRoom],
        defaultPaintPrices,
        defaultPricePerLiter,
        true,
      );
      const withoutMat = calculateEstimate(
        [simpleRoom],
        defaultPaintPrices,
        defaultPricePerLiter,
        false,
      );
      expect(withMat.rooms[0].labor_cost).toBeGreaterThan(
        withoutMat.rooms[0].labor_cost,
      );
    });
  ...
  describe("اسلایدر", () => {
    it("اسلایدر فقط روی اجرت تاثیر بذاره نه رنگ", () => {
      const result1 = calculateEstimate(
        [simpleRoom],
        defaultPaintPrices,
        defaultPricePerLiter,
        true,
