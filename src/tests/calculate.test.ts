...
import { calculateEstimate, PaintPrices, RoomInput } from "../utils/calculate";
...
describe("calculateEstimate", () => {
  describe("محاسبه مساحت", () => {
    it("مساحت دیوار رو درست حساب کنه", () => {
      const result = calculateEstimate([simpleRoom], defaultPaintPrices, 0);
  ...
  describe("محاسبه لیتر رنگ", () => {
    ...
    it("ضریب پوشش رنگ روغنی رو درست اعمال کنه", () => {
      const oilRoom: RoomInput = {
        ...simpleRoom,
        wall_paint_type: "oil",
      };

      const result = calculateEstimate([oilRoom], defaultPaintPrices, 0);
    ...
    it("ضریب پوشش آکریلیک رو درست اعمال کنه", () => {
      ...
      const result = calculateEstimate([acrylicRoom], defaultPaintPrices, 0);
    ...
    it("با تعداد دست بیشتر، لیتر بیشتری حساب کنه", () => {
      ...
      const result3 = calculateEstimate([threeCoats], defaultPaintPrices, 0);
      const result2 = calculateEstimate([twoCoats], defaultPaintPrices, 0);

      expect(result3.rooms[0].wall_paint_liters).toBeGreaterThan(
        result2.rooms[0].wall_paint_liters,
      );
    });
  });

  describe("محاسبه هزینه", () => {
    it("هزینه رنگ دیوار رو درست حساب کنه", () => {
      const result = calculateEstimate([simpleRoom], defaultPaintPrices, 0);

      // 12 لیتر * 700000 = 8,400,000
      expect(result.rooms[0].wall_paint_cost).toBe(12 * 700000);
    });

    it("هزینه اجرت رو درست حساب کنه", () => {
      const result = calculateEstimate(
        [simpleRoom],
        defaultPaintPrices,
        150000,
      ...
      // 50.4 * 150000 = 7,560,000
      expect(result.rooms[0].labor_cost).toBeCloseTo(50.4 * 150000);
    });

    it("وقتی قیمت رنگ صفره، هزینه رنگ صفر باشه", () => {
      const result = calculateEstimate([simpleRoom], {}, 0);

      expect(result.rooms[0].wall_paint_cost).toBe(0);
      expect(result.total_paint_cost).toBe(0);
    });

    it("ملزومات رو درست حساب کنه", () => {
      const result = calculateEstimate([simpleRoom], defaultPaintPrices, 0);

      // میانگین قیمت: (700000+850000+950000)/3 = 833333
      // base: 833333 * 2 = 1666666
      // perSqm: (833333/30) * 50.4 = 1400000
      expect(result.accessories_cost).toBeGreaterThan(0);
    });

    it("وقتی هیچ قیمتی نداری ملزومات صفر باشه", () => {
      const result = calculateEstimate([simpleRoom], {}, 0);
  ...
  describe("محاسبه روزهای کاری", () => {
    it("حداقل ۱ روز باشه", () => {
      ...
      const result = calculateEstimate([tinyRoom], defaultPaintPrices, 0);
    ...
    it("برای متراژ بزرگ، روزهای بیشتری حساب کنه", () => {
      ...
      const result = calculateEstimate([bigRoom], defaultPaintPrices, 0);

      // 2*(10+10)*2.8 = 112 متر → ceil(112/53) = 3 روز
